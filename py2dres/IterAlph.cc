...
void IterAlph::advection_alpha(double t, double* alpha, double** flux, double* new_alpha) {
  ...
  for (k=0; k<Nt; k++) {
    ...
    // 0 < alpha < 1
    new_alpha[k] = amMIN (ALPHA_MAX, amMAX(ALPHA_MIN, new_alpha[k]));
  }
}
...
void IterAlph::limit_grad(double** alpha) {
  ...
  for (k=0; k<Nt; k++) {
    ...
    for (seg=0; seg<3; seg++) {
      ...
      if( tri!=-1) {
        alpha_max=amMAX( alpha_max, alpha[0][tri] );
        alpha_min=amMIN( alpha_min, alpha[0][tri] );
      }
    }

    alpham=amMAX(alpha_min, amMIN(alpha[0][k], alpha_max));
    ...
    for (i=1; i<3; i++) {
      node = Coort[3*k+i];
      Dxmax = amMAX(Dxmax, (Coorp[2*node+0]-xc)*alpha_x );
      Dxmin = amMIN(Dxmin, (Coorp[2*node+0]-xc)*alpha_x );
      Dymax = amMAX(Dymax, (Coorp[2*node+1]-yc)*alpha_y );
      Dymin = amMIN(Dymin, (Coorp[2*node+1]-yc)*alpha_y );
    }
    ...
    phix=amMIN(Txmin, amMIN(Txmax, 1.0));
    /*printf("Tmax=%g, Tmin=%g, phi=%g\n",Txmax,Txmin,phix);
     */

    phiy=amMIN(Tymin, amMIN(Tymax, 1.0));
    /*printf("Tmax=%g, Tmin=%g, phi=%g\n",Tymax,Tymin,phiy);
     */
  ...
  for (k=0; k<Nt; k++) {
    ...
    for (seg=0; seg<3; seg++){
      ...
      if( tri!=-1) {
        ...
        alpha_x_max=amMAX( alpha_x_max, amx );
        alpha_x_min=amMIN( alpha_x_min, amx );
        alpha_y_max=amMAX( alpha_y_max, amy );
        alpha_y_min=amMIN( alpha_y_min, amy );
      }
    }

    alphamx = amMAX(alpha_x_min, amMIN(alpha[0][k], alpha_x_max));
    alphamy = amMAX(alpha_y_min, amMIN(alpha[0][k], alpha_y_max));
